			...
			if ( ! empty( $send_email_notifications ) && ! empty( $email_addresses_field ) ) {
				...
				if ( ! empty( $email_addresses ) ) {
					...
					foreach ( $email_addresses as $email ) {
						$subject = sprintf( __( '%s: Form Submission to "%s"', 'custom-contact-forms' ), wp_strip_all_tags( wp_specialchars_decode( get_bloginfo( 'name' ) ) ), wp_specialchars_decode( get_the_title( $form_id ) ) );
