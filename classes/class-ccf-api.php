...
class CCF_API_Form_Controller extends WP_REST_Controller {
	...
	*/
	public function get_item( $request ) {
		...
		$data = $this->prepare_item_for_response( $params->id );

		if ( is_array( $data ) ) {
			return new WP_REST_Response( $data, 200 );
		} else {
			return new WP_Error( 'code', __( 'Form not found', 'custom-contact-forms' ) );
		}
	}
	...
	*/
	public function create_item( $request ) {
		...
		$data = $this->prepare_item_for_response( $item_id );
	...
	*/
	public function get_items_permissions_check( $request ) {
		return true; //current_user_can( 'read_posts' );
	}
	...
	* @return mixed
	*/
	public function prepare_item_for_response( $item_id ) {
		$data = get_post( $item_id, ARRAY_A );

		$data['ID'] = (int) $item_id;

		$data['fields'] = $this->_get_fields( $item_id );

		$data['buttonText'] = esc_attr( get_post_meta( $item_id, 'ccf_form_buttonText', true ) );
		$data['description'] = esc_html( get_post_meta( $item_id, 'ccf_form_description', true ) );
		$data['completionActionType'] = esc_attr( get_post_meta( $item_id, 'ccf_form_completion_action_type', true ) );
		$data['completionRedirectUrl'] = esc_url_raw( get_post_meta( $item_id, 'ccf_form_completion_redirect_url', true ) );
		$data['completionMessage'] = esc_html( get_post_meta( $item_id, 'ccf_form_completion_message', true ) );
		$data['sendEmailNotifications'] = (bool) get_post_meta( $item_id, 'ccf_form_send_email_notifications', true );
		$data['pause'] = (bool) get_post_meta( $item_id, 'ccf_form_pause', true );
		$data['pauseMessage'] = esc_html( get_post_meta( $item_id, 'ccf_form_pause_message', true ) );
		$data['emailNotificationAddresses'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_addresses', true ) );
		
		$data['emailNotificationFromType'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_from_type', true ) );
		$data['emailNotificationFromAddress'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_from_address', true ) );
		$data['emailNotificationFromField'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_from_field', true ) );

		$data['emailNotificationSubjectType'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_subject_type', true ) );
		$data['emailNotificationSubject'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_subject', true ) );
		$data['emailNotificationSubjectField'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_subject_field', true ) );

		$data['emailNotificationFromNameType'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_from_name_type', true ) );
		$data['emailNotificationFromName'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_from_name', true ) );
		$data['emailNotificationFromNameField'] = esc_html( get_post_meta( $item_id, 'ccf_form_email_notification_from_name_field', true ) );

		$submissions = get_children( array( 'post_parent' => $item_id, 'numberposts' => array( 'ccf_max_submissions', 5000, $data ) ) );
